...
# v1.5 — two-sided (long + short). Symmetric confidence; sentiment gating retained.
# - Emits BUY when proba_up >= 0.5; emits SELL (short) when proba_up < 0.5 and SHORTS_ENABLED=1
# - score = max(proba_up, 1 - proba_up) so conf gate is symmetric
# - Long TP/SL: SL=entry*0.99, TP=entry*(1+0.01*R)
# - Short TP/SL: SL=entry*1.01, TP=entry*(1-0.01*R)
# - Confidence gate happens OUTSIDE the ML function; diagnostics show conf_gate vs no_signal cleanly
# - Introspect mode shows top probabilities, plus reason counters
...
APP_TAG = "Ranked_ML v1.5 (two-sided + outer_conf_gate)"
...
BYPASS_SESSION            = os.getenv("BYPASS_SESSION","0").lower() in ("1","true","yes")
...
# INTROSPECT mode (optional)
INTROSPECT         = os.getenv("INTROSPECT","0").lower() in ("1","true","yes")
...
class LiveTrade: ...  # (already defined above)
...
def ml_score(symbol: str, df1m: pd.DataFrame, tf_min: int):
    ...
    try:
        ...
        clf.fit(X.iloc[: int(len(X)*0.7) ], y.iloc[: int(len(y)*0.7) ])
        x_live = X.iloc[[-1]]
        try: x_live = x_live[list(clf.feature_names_in_)]
        except Exception: pass
        proba = float(clf.predict_proba(x_live)[0][1])  # P(up)
        ts = feats.index[-1]
        if not _in_session(ts):
            return proba, ts, "not_in_session"
        return proba, ts, None
    except Exception:
        return None, None, "model_error"

def signal_ml_pattern(symbol: str, df1m: pd.DataFrame, tf_min: int, r_multiple=None):
    """Two-sided candidate: returns BUY when proba_up>=0.5; SELL when proba_up<0.5 and SHORTS_ENABLED."""
    ...
    if proba_up is None or reason == "not_in_session":
        return None

    bars = _resample(df1m, tf_min)
    price = float(bars["close"].iloc[-1])

    # Decide side + score (symmetric confidence)
    if proba_up >= 0.5:
        side = "buy"
        score = proba_up
        sl = price * 0.99
    ...
    return {
        "action": side,                    # "buy" or "sell" (short entry)
        "orderType": "market",
        "price": None,
        "takeProfit": tp,
        "stopLoss": sl,
        ...
        "entry": price,
        "quantity": int(qty),
        "confidence": score,               # symmetric confidence
        "score": score,
        "meta": {"note": "ml_pattern", "confidence": score}
    }

# ============== Daily guard / diagnostics (unchanged) ==============
        ...
        try:
            ...
            if candidates:
                ...
                for score, sym, tf, sig in candidates:
                    ...
                    print(f"[ORDER] {combo_key} {sig['action']} -> qty={sig.get('quantity')} conf={score:.3f} ok={ok} info={info}", flush=True)
