...
# - NEW: confidence-ranked dispatch (highest confidence first) + ALLOW_ENTRIES toggle
...
ALLOW_ENTRIES          = os.getenv("ALLOW_ENTRIES","1").lower() in ("1","true","yes")  # pause switch
...
# Daily portfolio guard (duplicate in old file; keeping your values if you use them elsewhere)
START_EQUITY              = float(os.getenv("START_EQUITY", "100000"))
DAILY_TP_PCT              = float(os.getenv("DAILY_TP_PCT", "0.25"))  # keep consistent with above
...
def send_to_traderspost(payload: dict):
    """POST order payload to TradersPost or DRY-RUN/PAUSED."""
    try:
        if DRY_RUN or not ALLOW_ENTRIES:
            print(f"[DRY/PAUSED] {json.dumps(payload)[:500]}", flush=True)
            return True, "dry/paused"
        if not TP_URL:
...
def compute_sentiment():
    ...
    def _premarket_gap(sym):
        ...
        if len(dates) < 2:
            return 0.0
        yday = dates[-2]
        y_rth = df[(df.index.date == yday) & (df.index.map(_is_rth_tz))]
        if y_rth.empty:
            return 0.0
    ...
    if mode == "premarket_gap" and not is_rth_now:
        if avg >= gap_bp: out = "bull"
        elif avg <= -gap_bp: out = "bear"
        else: out = "neutral"
...
# ------------------------------
# Strategy: ML Pattern (live adapter) — now returns confidence/score
# ------------------------------
def signal_ml_pattern(symbol: str, df1m: pd.DataFrame, tf_min: int,
                      conf_threshold=None, r_multiple=None):
    """
    Lightweight live adapter:
    - Resample; build simple features; train RF; score last bar.
    - Long-only by default (sentiment gate filters side outside).
    Returns dict with 'confidence'/'score' for ranking.
    """
    try:
    ...
    proba = clf.predict_proba(x_live)[0][1]   # P(up)
    pred  = int(proba >= 0.5)
    ts = bars.index[-1]
    ...
    if pred == 1 and proba >= conf_threshold:
        ...
        return {
            ...
            "confidence": conf,
            "score": conf,
            "meta": {"note": "ml_pattern", "confidence": conf}
        }
    return None
...
# ------------------------------
# Router (single signal computation)
# ------------------------------
...
def handle_signal(strat_name: str, symbol: str, tf_min: int, sig: dict):
    ...
    print(f"[ORDER] {combo_key} -> qty={sig.get('quantity')} conf={conf if conf is not None else 'NA'} ok={ok} info={info}", flush=True)
...
def main():
    ...
    while True:
        ...
        try:
            ...
            # ---- SCAN PHASE: collect candidates (confidence) ----
            ...
            # ---- RANK & DISPATCH ----
            if candidates:
                candidates.sort(key=lambda x: x[0], reverse=True)  # highest confidence first
                sent = 0
                for score, sym, tf, sig in candidates:
                    if MAX_ENTRIES_PER_CYCLE > 0 and sent >= MAX_ENTRIES_PER_CYCLE:
                        break
                    ...
                    # Make confidence explicit in meta for audit
                    sig.setdefault("meta", {})
                    sig["meta"]["confidence"] = float(sig.get("confidence", score))
